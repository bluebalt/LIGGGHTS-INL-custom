...
COHESION_MODEL(COHESION_BOND2,bond2,16)
#else

#ifndef COHESION_MODEL_BOND2_H_
#define COHESION_MODEL_BOND2_H_
...
static const double SMALL_COHESION_MODEL_BOND2 = 1.e-6;

// required model properties
namespace MODEL_PARAMS
{
  static const char * DISSIPATION_NORM_FORCE_BOND2 = "dissipationNormalForceBond";
  static const char * DISSIPATION_TANG_FORCE_BOND2 = "dissipationTangentialForceBond";
  static const char * DISSIPATION_NORM_TORQUE_BOND2 = "dissipationNormalTorqueBond";
  static const char * DISSIPATION_TANG_TORQUE_BOND2 = "dissipationTangentialTorqueBond";

  inline static MatrixProperty* createBondmultiplier(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, "Bondmultiplier", caller, sanity_checks, 0.0);
  }

  /* ---------------------------------------------------------------------- */

  inline static MatrixProperty* createNormalBondStiffness2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
  ...
  inline static MatrixProperty* createTangentialBondStiffness2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
  ...
  inline static MatrixProperty* createDissipationNormalForceBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createDissipationMatrix(registry, DISSIPATION_NORM_FORCE_BOND2, caller, sanity_checks);
  }

  inline static MatrixProperty* createDissipationTangentialForceBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createDissipationMatrix(registry, DISSIPATION_TANG_FORCE_BOND2, caller, sanity_checks);
  }

  inline static MatrixProperty* createDissipationNormalTorqueBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createDissipationMatrix(registry, DISSIPATION_NORM_TORQUE_BOND2, caller, sanity_checks);
  }

  inline static MatrixProperty* createDissipationTangentialTorqueBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createDissipationMatrix(registry, DISSIPATION_TANG_TORQUE_BOND2, caller, sanity_checks);
  }

  /* ---------------------------------------------------------------------- */

  inline static MatrixProperty* createDampingNormalForceBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, "dampingNormalForceBond", caller, sanity_checks, 0.0, 1.0);
  }

  inline static MatrixProperty* createDampingTangentialForceBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, "dampingTangentialForceBond", caller, sanity_checks, 0.0, 1.0);
  }

  inline static MatrixProperty* createDampingNormalTorqueBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, "dampingNormalTorqueBond", caller, sanity_checks, 0.0, 1.0);
  }

  inline static MatrixProperty* createDampingTangentialTorqueBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
  ...
  /* ---------------------------------------------------------------------- */
  inline static MatrixProperty* createMaxDistanceBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, "maxDistanceBond", caller, sanity_checks, 0.0);
  }

  inline static MatrixProperty* createMaxSigmaBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, "maxSigmaBond", caller, sanity_checks, 0.0);
  }

  inline static MatrixProperty* createMaxTauBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, "maxTauBond", caller, sanity_checks, 0.0);
  }

  inline static MatrixProperty* createRatioTensionCompression2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createPerTypePairProperty(registry, "ratioTensionCompression", caller, sanity_checks, 0.0);
  }

  inline static MatrixProperty* createFrictionAngle2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
  ...
  inline static ScalarProperty* createTsCreateBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  {
    return createScalarProperty(registry, "tsCreateBond", caller, sanity_checks, 0.0);
  }

  // inline static MatrixProperty* createCreateDistanceBond2(PropertyRegistry & registry, const char * caller, bool sanity_checks)
  // {
...
namespace ContactModels {
  enum{
       BREAKSTYLE_SIMPLE2,
       BREAKSTYLE_STRESS2,
       BREAKSTYLE_STRESS_TEMP2
      };

  template<>
  class CohesionModel<COHESION_BOND2> : public CohesionModelBase {
  ...
  public:
    ...
    void postSettings(IContactHistorySetup * hsetup, ContactModelBase *cmb)
    {
      if (elasticpotflag_) 
      //탄성 잠재 에너지 초기화 
      //(elasticpotflag_)이미 등록된 히스토리 오프셋이 있는지 확인하고,
       //없다면 elastic_potential_cohesion과 관련된 힘·토크 성분을 위한 히스토리 슬롯을 추가합니다.
        //벽과의 접촉이면 추가로 elastic_potential_wall 항목을 만듭니다.마지막에 이 오프셋을 결합 모델에 연결합니다
      {
      ...
      if (dissipatedflag_)
      //소산 에너지 추적 준비 
      //(dissipatedflag_)접촉 대상이 벽인지에 따라 dissipated_energy_wall 또는 dissipated_energy fix를 찾아 FixPropertyAtom 포인터를 얻습니다.
    //벽 접촉 시에는 dissipation_force 히스토리 오프셋도 검색하며, 없으면 오류를 발생시킵니다.해당 fix를 찾지 못하면 “Surface model has not registered dissipated_energy fix” 오류를 던집니다
      {
        if (cmb->is_wall())
        {
    ...
    void connectToProperties(PropertyRegistry & registry)
    {
      ...
      registry.registerProperty("Bondmultiplier", &MODEL_PARAMS::createBondmultiplier);
      ...
      registry.connect("Bondmultiplier", bondmultiplier_,"cohesion bond");
      ...
      if (!stressbreakflag_)
      {
        ...
        registry.connect("maxDistanceBond", maxDist_,"cohesion bond2");
        breakmode_ = BREAKSTYLE_SIMPLE2;
        ...
        for (int i=1; i<max_type+1; i++)
        {
          for(int j=1;j<max_type+1;j++)
          {
            ...
            double cdf_one = 0*1.1 * 0.5*maxDist_[i][j]/minrad;
    ...
    void surfacesClose(SurfacesCloseData & scdata, ForceData & i_forces, ForceData & j_forces)
    {
      ...
      // const double lambda = bondmultiplier_[itype][jtype];
      ...
      if (update_history)
      {
          ...
          if (contflag[0] < SMALL_COHESION_MODEL_BOND)
          {
            ...
            const bool create_bond_A =  (createbond_always_flag_ || (update->ntimestep == static_cast<int>(tsCreateBond_))) && (r < (radi + radj) * (1.0 + bondmultiplier_[itype][jtype]));
            ...
            else if (fix_bond_random_id_)
            {
                ...
                const bool create_bond_B = bond_rand_id && !scdata.is_wall && (update->ntimestep < bond_rand_id[i]) && MathExtraLiggghts::compDouble(bond_rand_id[i],bond_rand_id[j]) && (r < (radi + radj) * (1.0 + bondmultiplier_[itype][jtype]));
      ...
      if (breakmode_ == BREAKSTYLE_SIMPLE2 && r > maxDist_[itype][jtype] && update_history)
      {
        breakBond(scdata);
        return; // bond broken
      }
      ...
      if (breakmode_ == BREAKSTYLE_STRESS2)
      {
