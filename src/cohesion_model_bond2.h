    ...
    void surfacesClose(SurfacesCloseData & scdata, ForceData & i_forces, ForceData & j_forces)
    {
      ...
      if (update_history)
      {
          ...
          if (contflag[0] < SMALL_COHESION_MODEL_BOND)
          {
            ...
            const bool create_bond_A =  (createbond_always_flag_ || (update->ntimestep == static_cast<int>(tsCreateBond_))) && (r_create < (radi + radj) * (1.0 + bondmultiplier_[itype][jtype]));
            ...
            else if (fix_bond_random_id_)
            {
                ...
                const bool create_bond_B = bond_rand_id && !scdata.is_wall && (update->ntimestep < bond_rand_id[i]) && MathExtraLiggghts::compDouble(bond_rand_id[i],bond_rand_id[j]) && (r_create < (radi + radj) * (1.0 + bondmultiplier_[itype][jtype]));
